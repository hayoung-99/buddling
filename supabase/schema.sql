...
begin
  ...
  values (coalesce(nullif(trim(p_name), ''), 'buddling'), v_code, now() + invite_ttl())
...
begin
  ...
  exception when others then
    raise notice '[buddling] pg_cron 을 켜지 못해 자동 정리를 걸지 않았습니다 (%). 대시보드 Database → Extensions 에서 켠 뒤 이 파일을 다시 실행하세요. 나머지 설정은 모두 적용됐습니다.', sqlerrm;
    return;
  end;
  ...
  raise notice '[buddling] 익명 계정 자동 정리를 매일 UTC 18시에 걸었습니다.';
end;
$$;
