...
주소는 `https://tap-tap-desktop.vercel.app/admin/` 하나입니다. `.vercel.app` 아래에는
...
`admin_daily(int)` · `admin_distribution()` 셋 다 첫 줄에서 `is_admin()` 을 확인하고
아니면 `FORBIDDEN` 으로 끝납니다. 그래서 **anon 키가 브라우저에 실려 있어도 집계가
새지 않습니다.** service role 키는 이 저장소 어디에도 두지 않습니다 — 그 키는 RLS 를
통째로 지나갑니다.

**집계만 내보냅니다.** 팀 이름·닉네임·초대코드는 어드민에도 나가지 않습니다. 사용률을
보는 데 필요하지 않고, 필요 없는 것을 볼 수 있게 만들면 그 화면이 유출 경로가 됩니다.
같은 이유로 **읽기 전용**입니다 — 고치거나 지우는 단추가 없습니다.

**어드민은 이메일로 정합니다.** `public.admins` 표에 주소를 넣어 두면 그 주소로 매직링크
로그인을 한 계정이 어드민이 됩니다. 표가 비어 있으면 아무도 못 들어갑니다.

```sql
insert into public.admins (email) values ('you@example.com');
```

**등록되지 않은 주소에는 메일이 아예 나가지 않습니다.** `restrict_admin_signup` 함수를
Supabase 대시보드의 **Authentication → Hooks → Before User Created** 에 걸어 둔 덕입니다
(걸지 않으면 이 함수는 아무 일도 하지 않습니다). 모르는 사람이 아무 주소나 넣어 링크를
요청할 때마다 계정이 하나씩 생기고 **시간당 두 통뿐인 메일 한도를 남이 써 버리는 것**을
막으려는 것입니다.
...
- `accounts` 는 사람 수가 아닙니다. 익명 계정은 앱을 다시 깔 때마다 늘어납니다.
  사람 수는 `people`(팀에 속한 서로 다른 계정) 입니다
...
**앱이 하루에 두 번 흔적을 남깁니다** (`touch_member()`, `main/session.ts` 의
`TOUCH_INTERVAL_MS`). 이 앱은 켜 두고 쓰는 앱이라 앱을 켤 때만 남기면 **가장 오래 쓰는
사람이 가장 활동 없어 보입니다.** 어드민의 활동자 수가 이 흔적 위에 서 있으므로
주기를 늘리면 그 숫자도 함께 흐려집니다.
