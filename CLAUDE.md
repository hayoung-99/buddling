...
README 와 기획서가 어긋나 보이면 **기획서가 이깁니다.** README 는 지금 무엇이
어떻게 되어 있는지를 적고, 기획서는 그것이 왜 그런지와 어디로 갈지를 적습니다.
(실제로 어긋난 적이 있습니다 — README 는 "모델 파일이 하나도 없다" 를 자랑처럼
적어 두었는데, 기획서에서 그것은 **지금의 구현 선택이지 지켜야 할 원칙이 아니고**
언젠가 Blender 로 옮기는 것이 목표라고 정해 두었습니다.)
...
- **`packages/shared` 만 고친 커밋은 릴리스에 안 잡힙니다.** release-please 는 커밋을
  ...
  **사전만 고쳤다면 앱 쪽 변경과 함께 내보내거나, 머지 뒤 릴리스 PR 이 생겼는지 보세요.**
  ...
  | 시도 | 막힌 이유 |
  |---|---|
  | `node-workspace` 플러그인 | 의존하는 쪽의 범위 표기를 정확한 버전으로 고쳐 씁니다. `apps/desktop` 이 적어 둔 것이 `"*"` 라 접두어가 없어 통째로 박히는데, `@tap-tap/shared` 는 private 이라 npm 에 없어서 **`npm ci` 가 404 로 죽습니다** (재현해 봤습니다) |
  | `linked-versions` 플러그인 | 묶음을 **컴포넌트 이름**으로 찾는데, 앱은 `include-component-in-tag: false` 라 그 이름이 빈 문자열이라 안 잡힙니다. 이름을 붙이면 태그가 `tap-tap-v0.5.0` 이 되는데 **`electron-builder --publish always` 는 태그를 스스로 `v{버전}` 으로 정하므로** 릴리스가 둘로 갈립니다 |
