...
**규칙셋에 필수 검사(required status checks)는 없습니다.** CI 가 빨개도 머지는
막히지 않습니다. 초록인지 확인하는 것은 사람과 에이전트의 몫입니다.
...
| | 어떻게 |
|---|---|
| `chore(deps)` · dependabot 갱신 | CI 초록을 **확인한 뒤** `gh pr merge <번호> --squash` |
| 오타·문구·주석만 고친 `docs:` | 같음 |
| 그 밖 전부 | **PR 을 열고 멈춘다.** 사람이 누른다 |

초록을 확인하고 나서 누르는 것이 중요합니다. 위에 적었듯 규칙셋이 검사를 강제하지
않으므로, 빨간 채로도 머지가 됩니다.

**`gh pr merge --auto` 는 지금 쓸 수 없습니다.** 저장소 설정에서 auto-merge 가 꺼져
있어(`allow_auto_merge: false`) 그 명령은 거부됩니다. 켜고 싶다면 저장소 설정에서
"Allow auto-merge" 를 켜면 되고, 그러면 CI 를 지켜보다 초록이 되는 순간 GitHub 이
스스로 머지합니다. 그때까지는 위 표대로 **확인하고 누르는** 방식입니다.
