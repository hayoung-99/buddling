 ...
 *
 * **세로를 늘릴 때는 화각과 짝으로 움직여야 한다.** 세로 화각이 고정이라
 * (`renderer/pet/scene.ts` 의 `PET_CAMERA.fov`) 높이만 늘리면 보이는 범위는 그대로인
 * 채 캐릭터만 커진다. 수줍음의 말풍선을 넣을 때 위가 모자라 둘을 함께 키워 봤는데,
 * 말풍선을 머리 한가운데가 아니라 한쪽으로 비켜 세우니 지금 크기에 들어와서 되돌렸다.
 * 다음에 세로가 모자라면 이 짝을 기억할 것.
 */
