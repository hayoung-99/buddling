 ...
 * 팔이 올라오기 시작한 뒤에 뜬다(초).
 *
 * `SHY_UNIT` 은 0.22초에 팔이 올라오기 시작해 0.44초에 얼굴 옆에 닿는다. 그 사이에
 * 떠야 **팔을 올린 것 때문에 말풍선이 생긴 것**으로 읽힌다. 처음부터 띄우면 둘이
 * 따로 노는 두 가지 일이 된다.
 */
const DELAY = 0.3
 ...
 *
 * **머리 한가운데가 아니라 한쪽으로 비켜 세운다.** 가운데에 두었더니 귀가 긴 홉 버니
 * 에서 두 귀 사이에 끼어, 떠 있는 말풍선이 아니라 귀에 붙은 판으로 보였다. 비켜
 * 세우면 다섯 종 모두 머리 곁에 뜨고, 팔을 든 쪽의 반대편이라 창 안이 좌우로 균형을
 * 잡는다 (인사 짝대기와 같은 판단이다).
 */
