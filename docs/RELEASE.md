...
> 머지하면 안 됩니다. 초안 릴리스에는 아직 git 태그가 없어서, release-please 가
> 초안을 만든 직후 계산하는 "다음 릴리스 PR" 은 기준점을 잃고 역사를 처음부터 다시
> 훑습니다. 그러면 버전이 뒤로 가는 제안이 나옵니다 — 릴리스 네 번 내내 그랬습니다.
> 워크플로의 `refresh` 잡이 태그가 생긴 뒤 다시 계산해 바로잡지만, 그래도 눈으로
> 한 번 확인하는 편이 안전합니다.
...
규칙은 [`src/main/update-schedule.js`](../src/main/update-schedule.js) 한 곳에 있고
